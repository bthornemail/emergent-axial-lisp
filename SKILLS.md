# EMERGENT AXIAL LISP: EXECUTE-ONLY COGNITIVE SKILLS MATRIX
# CONFIGURATION: LOCAL WIRE AUDITING HARNESS

## [SKILL_01: RUN_LOCAL_VERIFY_HARNESS]
### OBJECTIVE: Execute type-custody checks, local wire law verifications, and C99 bare-metal syntax bounds
### INVARIANTS:
* Verification Target == make verify
* Substrate Core Count == Exactly 23 flat files under ./.omi/
* Extraction Invariant == (car (cons x y)) == x and (cdr (cons x y)) == y
### RUN:
```text
INPUT: current_word_context, active_field_axis, inbound_block_token
FORBIDDEN: Heap allocation pointers, variable list recursion loops, dynamic timing channels, unverified C99 drift
...
COMPUTE:
  ...
  # 4. Trigger the local strict verification harness running C99 and Haskell checks to lock state
